...
if(isset($_POST['update_class_subject'])){
    ...
    if($response){
        ...
        window.location.href='view_class_subject.php';
        </script>";
    } else {
...
// Fetch existing data
if(isset($_GET['id'])){
    $ObjData = Settings::get_class_subjects($conn, $_GET['id']);

    if($ObjData){
        $ID        = $ObjData[0]->id;
        $classID   = $ObjData[0]->class_id;
        $subjectID = $ObjData[0]->subject_id;
        $ButtonValue = 'Update';
    }
}
...
$subjects_result = mysqli_query($conn, "SELECT id, name FROM subject");
?>
      ...
      <div class="card-style mb-30">
        ...
        <div class="d-flex align-items-center justify-content-between mb-25 p-3"
          ...
          <h4 style="margin:0; font-weight:600;">
            <i class="lni lni-graduation me-2"></i> Update Class Subject
          </h4>
        </div>
        ...
        <form method="POST">
          ...
          <div class="select-style-1">
            ...
            <div class="select-position">
              <select name="subject_id" required>
                ...
                <?php while($subject = mysqli_fetch_object($subjects_result)){ ?>
          ...
          <div class="button-group mt-3 text-center">
            <button type="submit" name="update_class_subject" class="main-btn primary-btn btn-hover">
